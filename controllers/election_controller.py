...
def results(election_id):
    ...
    if not election:
        logging.error(f"Election {election_id} not found.")
    ...
    # Early election determination logic
    eed_percentage = None
    likely_winner = None
    if election.is_early_determination():
        # Determine the likely winner
        likely_winner = max(election.candidates, key=lambda c: len(c.votes)).name
        eed_percentage = (len(likely_winner.votes) / total_votes) * 100 if total_votes > 0 else 0

    return render_template("results.html", results=results_percentage, election_name=election.election_name, eed_percentage=eed_percentage, likely_winner=likely_winner)
